...
def update_phase_Phase_dialog(rep):
    if rep.IsSecurityEnabled:
        dialog_result = messagebox.askyesno(
            "Tevel EA Addin: Update Phase and Phase of Diagram Elements",
            "This option opens the Update Phase and Phase dialog box which enables update a phase and a Phase of the selected diagram's elements which are not locked.\n\nWould you like to proceed?",
            icon="warning"
        )
    else:
        dialog_result = messagebox.askyesno(
            "Tevel EA Addin: Update Phase and Phase of Diagram Elements",
            "This option opens the Update Phase and Phase dialog box which enables update a phase and a Phase of the selected diagram's elements.\n\nWould you like to proceed?",
            icon="warning"
        )
    ...
    if dialog_result:
        ...
        if sel_diagram:
            change_phase_Phase(rep, sel_diagram)
...
def change_phase_Phase(rep, selected_diagram):
    root = tk.Tk()
    root.title("Update Phase")

    label = tk.Label(root, text="Update the Phase to:")
    label.grid(row=0, column=0)

    txt_phase_Phase = tk.Entry(root)
    txt_phase_Phase.grid(row=0, column=1)

    btn_update = tk.Button(root, text="Update", command=lambda: on_update_click(rep, selected_diagram, txt_phase_Phase, root))
    btn_update.grid(row=1, column=1)

    root.mainloop()

def on_update_click(rep, selected_diagram, txt_phase_Phase, root):
    for cur_object in selected_diagram.DiagramObjects:
        ...
        if rep.IsSecurityEnabled:
            if not is_element_locked(cur_element.ElementGUID, rep):
                ...
                cur_element.Phase = txt_phase_Phase.get()
                cur_element.Update()
        else:
            cur_element.Phase = txt_phase_Phase.get()
            cur_element.Update()
