...
# Define constants
ACTOR_TEMPLATE = "Connectors"
USECASE_TEMPLATE = "Connectors"
            ...
            for i in range(package_elements.Count):
                ...
                if current_element.Type != None:
                    ...
                    for j in range(element_connectors.Count):
                        ...
                        if connected_element.Type == "UseCase":
                            # Generate Use Case documentation
                            report_info("Generating documentation for connected UseCase: " + connected_element.Name)
                            generation_success = doc_generator.DocumentElement(connected_element.ElementID, 1, USECASE_TEMPLATE)
                            if not generation_success:
                                report_warning("Error generating UseCase documentation: " + doc_generator.GetLastError())
                else:
                    report_info("Skipping element " + current_element.Name + " - not an actor")
